...
{
	if (g_Config.m_ClBubblePlay)
	{	
		static int LastInput[10];
		int InputList[10];
		...
		if (true &&
			InputList[6] != 4 && InputList[6] != 2 && !InputList[5] &&
			LastInput[8] == InputList[8] && LastInput[9] == InputList[9] &&
			!InputList[3])
		{
			CMsgPacker Msg(NETMSG_INPUT);
			Msg.AddInt(m_AckGameTick + 1);
			Msg.AddInt(-1);
			Msg.AddInt(40);
